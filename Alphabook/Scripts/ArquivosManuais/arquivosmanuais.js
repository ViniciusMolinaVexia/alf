...
gafisa.alphabook.home.arquivosManuais = {
    ...
    mostrarModalEdicao: function (id, DataAlteracao, nomeArquivos) {
            ...
            success: function (response) {
                ...
                $.each(response, function (index, item) {
                    ...
                    let Status = gafisa.alphabook.home.arquivosManuais.getStatusClass(item.Status);
                        ...
                        var linha = '<tr>' +
                            '<td>' + item.ID + '</td>' +  // Assumindo que o item tem a propriedade 'id'
                            '<td>' + item.NomeArquivo + '</td>' + // Assumindo que o item tem a propriedade 'nome'
                            '<td>' + item.NomeUsuario + '</td>' + // Assumindo que o item tem a propriedade 'usuarioCarga'
                            '<td>' + item.MesInsercao + '</td>' +  // Assumindo que o item tem a propriedade 'id'
                            '<td>' + Data + '</td>' + // Assumindo que o item tem a propriedade 'dataCarga'
                            '<td><span class="status-circle ' + Status + '"></span></td>' + // Adiciona o span com a classe do status
                            '<td style="width:11%">' +
                ...
                $('.btn-edit').off('click').on('click', function () {
                    ...
                    // Chame a função que você deseja com o ID e a DataInsercao
                    gafisa.alphabook.home.arquivosManuais.mostrarModalEdicao(id, dataInsercao, nomeArquivo);
                });
